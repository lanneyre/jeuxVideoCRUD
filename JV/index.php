<?php 
	...
	try {
		...
		$queryJeux = $bdd->query("SELECT * FROM `Jeux` JOIN `Genre` ON (`Jeux`.`Genre_Id` = `Genre`.`Genre_Id`) ORDER BY `Jeux_Id` ASC");
		$queryGenre = $bdd->query("SELECT * FROM `Genre`");
		$queryPlateforme = $bdd->query("SELECT * FROM `Plateforme`");
		...
		<div class="row">
			<div class="col-12">
				<table class="table table-striped table-hover" id="jeuxVideo">
					...
					<tbody>
						<!-- // Partie a remplir avec les données -->
						<?php while ($jeuxVideo = $queryJeux->fetch()) { ?>
						<tr>
								...
								<div class="modal fade" id="ModalEdit<?php echo $jeuxVideo['Jeux_Id'];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								  <div class="modal-dialog" role="document">
								    <div class="modal-content">
								    	<form name="AddJeuVideo" method="POST" action="traitements.php">
									      <div class="modal-header">
									        <h4 class="modal-title" id="exampleModalLabel">Modifier un jeu</h4>
									        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
									          <span aria-hidden="true">&times;</span>
									        </button>
									      </div>
									      <div class="modal-body">
												...
												<label for="Genre_Id">Genre</label>
									      		<select name="Genre_Id" id="Genre_Id" class="form-control">
									      			...
									      			$queryGenre = $bdd->query("SELECT * FROM `Genre`");
									      			while($genre = $queryGenre->fetch()){
									      				?><option value="<?php echo $genre['Genre_Id'];?>" <?php if($genre['Genre_Id'] == $jeuxVideo['Genre_Id']) echo " selected";?>><?php echo $genre['Genre_Titre'];?></option><?php
									      			} ?>
									      		</select>
												Plateformes <br>
												<?php
													...
													while($Plateforme = $queryPlateforme->fetch()){
									      				?><div class="form-check form-check-inline">
									      					<input 
									      						...
									      						type="checkbox" 
									      						name="Plateformes[]" 
									      						id="Plateforme<?php echo $Plateforme['Plateforme_id'];?>" 
									      						value="<?php echo $Plateforme['Plateforme_Id'];?>"
									      						<?php if(in_array($Plateforme['Plateforme_Id'], $JeuxPlateforme)) echo " checked"; ?>>
									      					<label class="form-check-label" for="Plateforme<?php echo $Plateforme['Plateforme_id'];?>"><?php echo $Plateforme['Plateforme_Nom'];?></label>
									      				</div><?php 
									      			} ?>
